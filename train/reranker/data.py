...
class GroupedRankerDatasetWithPriorAttention(GroupedRankerDataset):
    """
    Extends GroupedRankerDataset by adding prior-attention support.

    Output:
      tokens,
      {
        "labels": Tensor[N],
        "prior_attention": Tensor[N]
      }
    """
    ...
    def read_data(self, data_path):
        ...
        with open(data_path, "r", encoding="utf-8") as f:
            for line in tqdm.tqdm(f, desc=f"Loading {self.tag}+prior attention"):
                row = json.loads(line)
                ...
                for i in range(0, len(hits), self.train_group_size):
                    ...
                    for h in group:
                        ...
                        prior_attention.append(float(h["prior_attention"]))
                    ...
                    data.append([query, docs, labels, prior_attention])

        if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
            print(f"Loaded {len(data)} grouped samples with prior attention")

        return data

    def collate_fn(self, batch):
        all_pairs, all_labels, all_prior_attention = [], [], []

        for query, docs, labels, prior_attention in batch:
            for d, l, attention in zip(docs, labels, prior_attention):
                all_pairs.append([query, d])
                all_labels.append(l)
                all_prior_attention.append(attention)

        tokens = self.model.preprocess(all_pairs, self.max_len)

        return tokens, {
            "labels": torch.tensor(all_labels, dtype=torch.float32),
            "prior_attention": torch.tensor(all_prior_attention, dtype=torch.float32),
        }
