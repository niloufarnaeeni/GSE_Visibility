...
_REQUIRED_SKILL_HEADING_RE = re.compile(
    r"^\s*Required skill keywords:\s*$",
    re.IGNORECASE,
)
...
def extract_required_skill_keywords_from_query(query_text: str) -> List[str]:
    """
    Extract bullet points under the v5 "Required skill keywords:" section.
    """
    lines = str(query_text).splitlines()
    in_section = False
    keywords: List[str] = []

    for line in lines:
        if not in_section:
            if _REQUIRED_SKILL_HEADING_RE.match(line):
                in_section = True
            continue
...
def load_skill_keyword_index_map(
    skill_keywords_csv: Path,
    s2i: dict,
) -> Dict[str, int]:
    """
    Map normalized exact gpt5 skill keywords to sparse skill indices.
    """
    out: Dict[str, int] = {}

    with open(skill_keywords_csv, "r", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        required = {"id", "keyword"}

        if not required.issubset(reader.fieldnames or []):
            raise KeyError(
                "gpt5_skills.csv must contain columns: "
                f"{sorted(required)}"
            )
        ...
        for row in reader:
            ...
            if skill_id not in s2i:
                log.warning(
                    "Skill ID %s from %s is missing from indexes['s2i'].",
                    skill_id,
                    skill_keywords_csv,
                )
                continue

            out[_normalize_skill_keyword(keyword)] = int(s2i[skill_id])

    return out


def extract_required_skill_indices(
    query_text: str,
    s2i: dict,
    keyword_to_skill_index: Optional[Dict[str, int]] = None,
) -> List[int]:
    req = extract_required_skill_indices_from_query(query_text, s2i)

    if req:
        return req

    if keyword_to_skill_index is None:
        return []

    indices: List[int] = []
    unmapped: List[str] = []

    for keyword in extract_required_skill_keywords_from_query(query_text):
        normalized = _normalize_skill_keyword(keyword)
        idx = keyword_to_skill_index.get(normalized)
    ...
    if unmapped:
        log.warning(
            "Unmapped required skill keywords: %s",
            unmapped,
        )
...
def compute_skill_coverage_at_k(
    ...
    ks: Tuple[int, ...] = (2, 5, 10),
    skill_keywords_csv: Optional[Path] = None,
    ...
    """
    s2i = indexes["s2i"]
    keyword_to_skill_index = (
        load_skill_keyword_index_map(Path(skill_keywords_csv), s2i)
        if skill_keywords_csv is not None
        else None
    )
